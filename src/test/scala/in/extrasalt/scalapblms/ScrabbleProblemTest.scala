...
class ScrabbleProblemTest extends FlatSpec {
  ...
  it should "return a word value for string starting at 0, 0 R" in {
    ScrabbleProblem.getWordValue("indix", new Score(Score.defaultScoreMap)) should be(13)

  }

  it should "modify score map for positive direction" in {
    val modifiedMap  = ScrabbleProblem.modify("indix", new Score(Score.defaultScoreMap), (0,0), "R")
    val modifiedMap2 = ScrabbleProblem.modify("unconsciousness", new Score(Score.defaultScoreMap),(0,0), "R")
    val modifiedMap3 = ScrabbleProblem.modify("indix", new Score(Score.defaultScoreMap),(1,1), "R")
  ...
  it should "modify score map for negative direction" in {
    val modifiedMap  = ScrabbleProblem.modify("indix", new Score(Score.defaultScoreMap), (7,8), "D")
    val modifiedMap2 = ScrabbleProblem.modify("unconsciousness", new Score(Score.defaultScoreMap),(14,14), "D")
